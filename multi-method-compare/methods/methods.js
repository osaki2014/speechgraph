...
export const METHODS=[
  {
    ...
    async run(ctx){
      const nodes=[]; const speech=await webSpeechAgent(ctx.sharedText||ctx.demoText||''); nodes.push(asNode('Speech Agent',speech,speech.value));
      const semantic=await semanticAnchorAgent(speech.value); nodes.push(asNode('Semantic Agent',semantic,`x=${semantic.value.x.toFixed(2)}, y=${semantic.value.y.toFixed(2)}`));
  ...
  },
  {
    ...
    async run(ctx){
      ...
      const integrated=await integrationLLMAgent({emotion:emo.value,inquiry:inq.value,science:sci.value,embeddingPreview:emb.value.slice(0,16)},ctx.llmEndpoint); nodes.push(asNode('Integration Agent',integrated,integrated.value.explanation||'integrated'));
